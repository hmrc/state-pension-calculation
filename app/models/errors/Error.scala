...
object ApiServiceError extends Error("INTERNAL_SERVER_ERROR", "An error occurred whilst processing your request.")

object InvalidRequestError extends Error("INVALID_REQUEST", "The request is invalid.")
...
object RetirementAfterDeathError
    extends Error(
      "RETIREMENT_DATE_AFTER_DEATH_DATE",
      "The remote endpoint has indicated that the Date of Retirement is after the Date of Death."
    )

object TooEarlyError
    extends Error(
      "TOO_EARLY",
      "The remote endpoint has indicated that the pension calculation can only be done within 6 months of the SPA date."
    )
...
object MatchNotFoundError
    extends Error(
      "NOT_FOUND_MATCH",
      "The remote endpoint has indicated that there is no match for the person details provided."
    )

object ServiceUnavailableError extends Error("SERVER_ERROR", "Service unavailable.")

object ThrottledError extends Error("MESSAGE_THROTTLED_OUT", "The application has reached its maximum rate limit.")
